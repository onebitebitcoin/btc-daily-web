...
| | `media.credit` | str \| null | 삽화면 ✓ | 기사 사진이 아닌 **자료 사진**일 때의 출처 표기. `find_illustration.py` 가 만들어 주는 문자열을 그대로 넣는다. `push_edition.py` 가 삽화 호스트를 알아보고 credit 없이 나가는 것을 막는다 (5.0절 4번) |
| `closing` | `eyebrow` | str | ✓ | |
...
- **`title`**: 펀치라인형 헤드라인, 한 문장. **명사로 끝낸다** — 2.1절 참고.
  ("연준 금리 동결에도 밀린 비트코인")
...
| 쓰지 말 것 | 쓸 것 |
|---|---|
| "비트코인이 6만4000달러를 되찾았다" | "6만4000달러를 되찾은 비트코인" |
| "인증을 미룬 계정은 30일 뒤 청산된다" | "인증 미룬 계정에 걸린 30일 시한" |
| "통제권은 어디로 가는가" | "토큰화가 바꾸지 못하는 것" |

- 서술형 종결(`~다`/`~한다`/`~했다`/`~이다`)과 의문형 종결(`~는가`/`~인가`/`~까`)을
  쓰지 않는다. 앞선 발행분 230장 중 77%가 `~다`로 끝나 제목이 매일 같은 리듬으로
  읽혔다 — 명사형은 그 단조로움을 깨는 장치이기도 하다.
- 관형절로 주어를 수식해 끝내는 형태가 기본이다("…한 비트코인", "…을 노린 유출").
- 대구를 쓸 때는 쉼표로 끊고 뒤를 명사로 닫는다("매수 멈춘 스트래티지, 반등 만든 파생상품").
- `push_edition.py`의 문구 게이트가 `TITLE_RULE_DATE`(2026-08-18) 이후 발행분에
  한해 이 규칙을 강제한다. 그 이전 발행분은 서술형이라 소급 적용하지 않는다.
...
카드뉴스가 글 모음처럼 보인다. 아래 4.0절 순서를 먼저 다 밟고, 그래도 쓸 이미지가
없을 때만 `media: null`로 간다.

### 4.0. 어떤 이미지를 고르는가 — 우선순위

`collect_daily.py`가 넘겨주는 `image_url`은 **후보지 정답이 아니다.** 2026-08-19~23
발행 48장을 전수 점검했더니 23%가 기사와 무관한 그림이었다(채굴 카드에 테더 로고,
ETF 카드에 채굴기 사진, 보안 카드에 구글 독스 아이콘). 매체가 기사마다 이미지를
성의껏 고르지 않기 때문이다 — 특히 토큰포스트 본문 이미지는 `alt` 크레딧이
`/ TokenPost.ai`인 자체 AI 생성물이라 기사 내용과 느슨하게만 이어진다.

그래서 아래 순서로 고른다. 위에서 걸리면 거기서 멈춘다.

1. **링크한 기사 본문의 실사진.** `<figcaption>`이 붙은 이미지가 실사일 확률이 높다
   (예: `캡션: 레이 달리오. 자료=CNBC 화면 갈무리`). `og:image`가 AI 생성이어도
   본문 안에는 진짜 사진이 있는 경우가 있으니 본문을 열어 확인한다.
2. **같은 기업·인물을 찍은 다른 기사의 실사진.** 카드가 다루는 대상 자체를 찍은
   사진이면 다른 기사에서 가져와도 된다(예: 코인베이스 수익 카드에 다른 기사의
   브라이언 암스트롱 CNBC 화면). 링크와 이미지 출처가 갈리지만, 무관한 스톡보다
   낫다. **사건이 아니라 대상이 같아야 한다** — 아무 비트코인 사진이나 붙이지 말 것.
3. **후보가 준 `image_url`.** 기사와 실제로 맞는지 눈으로 보고 판단한다.
4. **키워드 삽화.** 위 셋이 다 안 되면 주제를 떠올리게 하는 자료 사진을 붙인다.
   `backend/scripts/find_illustration.py "검색어"` 가 라이선스를 걸러 후보를 준다.
   출력의 `image` 를 `media.image` 에, `credit` 을 `media.credit` 에 **그대로** 넣는다.

   삽화는 **기사 사진이 아니다.** 2번 항목("같은 대상을 찍은 다른 기사의 사진")과
   헷갈리지 마라 — 2번은 그 대상을 실제로 찍은 보도 사진이고, 4번은 주제를
   설명하는 자료 사진이다. 그래서 카드에 출처가 찍히고, 독자도 사건을 찍은
   사진이 아니라는 것을 안다.

   두 가지를 지킨다.
   - **출처 표기는 의무다.** CC BY·BY-SA 는 저작자 표시가 라이선스 조건이다.
     `push_edition.py` 의 삽화 게이트가 credit 없는 삽화를 POST 전에 막는다.
   - **인물 사진을 일반 삽화로 쓰지 마라.** 스크립트가 `[인물?]` 로 표시해 준다.
     카드 주인공 본인이 아니면 고르지 않는다 — 실측으로 "data center servers"
     질의에 `Shaku Atre in 2016`(실존 인물 초상)이 올라왔다.

   **사건을 특정하는 카드에는 쓰지 마라.** "누가 무엇을 발표했다", "이 사람이
   이렇게 말했다" 류는 삽화가 증거처럼 읽힌다. 그런 카드는 5번으로 간다.
5. **`media: null`.** 위 셋이 다 안 되면.

거르는 것:
- **어둡고 위협적인 그림은 쓰지 않는다.** 기사가 하락·규제·해킹을 다뤄도 그림까지
  공포를 밀어붙일 이유가 없다. 아래에 하나라도 해당하면 다음 후보로 넘어간다.
  - 검은 연기, 불길, 폐허, 갈라진 땅, 무너지는 구조물
  - 핏빛 붉은 배경에 큰 하락 화살표를 얹은 구성
  - 찢긴 종이·부서진 동전·깨진 유리처럼 파손을 강조한 콜라주
  - 얼굴을 가린 인물, 수갑, 쇠사슬, 감옥, 무기
  - 화면 전체가 어두워 축소했을 때 형태가 안 읽히는 그림
  CryptoSlate 가 기사마다 붙이는 AI 콜라주가 이 계열에 자주 걸린다 — `og:image` 가
  그렇다고 그대로 쓰지 말고 4.0 절 순서를 다시 밟는다. 밝고 형태가 단순한 삽화
  (코인텔레그래프 계열의 팝아트 톤 등)나 실사진 쪽이 카드뉴스 톤에 맞는다.
  **1번 카드는 특히 엄격하게 본다** — 링크 공유 미리보기(`meta.og_image`)가 비어
  있으면 그 그림이 그대로 썸네일이 되고, 카드를 안 열어 본 사람에게 오늘자 인상을
  결정한다(2026-09-10: 유가 배럴·검은 연기·붉은 하락 화살표 콜라주가 썸네일로
  나갔다가 교체했다).
- **AI 생성 그림 중 글자가 깨진 것은 반드시 뺀다.** 화면·간판에 뭉개진 가짜 텍스트가
  박혀 있으면 확대했을 때 품질을 깎는다(실측: `SUH`·`WBTH` 같은 오타가 그대로 노출).
