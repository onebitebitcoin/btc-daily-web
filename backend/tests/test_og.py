...
def test_card_og_uses_that_cards_title_and_image(client) -> None:
    ...
    assert f'og:url" content="http://testserver/d/{CARD_DATE}/3"' in body
...
def test_edition_og_is_unchanged_without_card_index(client) -> None:
    ...
    assert f'og:url" content="http://testserver/d/{CARD_DATE}"' in body
...
def test_card_og_image_serves_that_cards_source(client, tmp_path, monkeypatch) -> None:
    _seed_cards_with_distinct_images(client.session_factory)
    ...
    response = client.get(f"/api/og/{CARD_DATE}/3/image.jpg")

    assert response.status_code == 200
    # 캐시 파일명은 원본 URL 지문으로 갈린다 — 카드 3의 그림으로 구워졌는지 확인한다.
    fingerprint = source_fingerprint("https://cdn.example/card-3.jpg")
    assert (tmp_path / f"{CARD_DATE}-{fingerprint}.jpg").exists()


def test_card_og_image_404_for_unknown_date(client, tmp_path) -> None:
    client.app.dependency_overrides[get_settings] = lambda: Settings(og_cache_dir=str(tmp_path))
